...
public class RobotRulesParser{
  ...
  public void setConf() {
    ...
    //
    String agentName = "google spider";
    String agentNames = "*";
      ...
      if ( (line.length() >= 11) 
        ...
        try {
          path= URLDecoder.decode(path, CHARACTER_ENCODING);
            ...
            System.out.println("error parsing robots rules- can't decode path: " + path);
        ...
        if (addRules) {
          ...
          if (delay.length() > 0) {
            try {
              ...
              System.out.println("can not parse Crawl-Delay:" + e.toString());
            }
  ...
  private RobotRuleSet getRobotRulesSet(HttpBase http, URL url) {
    ...
    if (robotRules == null) {                     // cache miss
      ...
      try {
        Response response = http.getResponse(new URL(url, "/robots.txt").toString(),
                                              true);
        ...
        if (response.getCode() == 301 || response.getCode() == 302) {
          ...
          if (redirection != null) {
            ...
            response = http.getResponse(redir.toString(), true);
          }
        }
        ...
        if (response.getCode() == 200)               // found rules: parse them
          ...
          System.out.println("Couldn't get robots.txt for " + url + ": " + t.toString());
