...
public class Tools {
	...
	public static String get_one_item(byte[] usedata, String charset,
			String xpath) {
	...
	public static String post_function(DownloadFunction downFunction,
			MedicalContent mcontent, String baseUrl, String postString) {
	...
	public static String trimFirstAndLastChar(String source, char element) { // 去除字符串中首尾指定字符
		...
		do {
			...
			int endIndex = source.lastIndexOf(element) + 1 == source.length()
	...
	public static String get_domin_name(String url) {
		...
		if (url_object != null) {
			if (url_object.getPort() != -1)// url_object.getPort()=-1
				baseurl = url_object.getProtocol() + "://"
						+ url_object.getHost() + ":" + url_object.getPort();// url_object.getProtocol()=http
			else
				baseurl = url_object.getProtocol() + "://"
						+ url_object.getHost();// url_object.getHost()=gsxt.scaic.gov.cn
		}
	...
	public static String EncodeUtf8ByteToString(byte[] buffer) {
		...
		while (count < utfLength) {
			if ((result[index] = (char) buffer[count++]) < 0x80) {
				...
				result[index++] = (char) (((a & 0x0F) << 12) | ((b & 0x3F) << 6)
						| (c & 0x3F));
	...
	/**
	 * 
	 * @Title: gettime @Description: 获取Unix时间戳 @author qdcz @throws
	 */
	...
	public static void main(String[] args) {
		...
		getMultiResultsByOneXpathPattern(str.getBytes(), "utf-8",
				"//ul[@class='txtbox clearfix']//li/a/@href", urls);
		for (String url : urls) {
	...
	// 一个xpathparser返回一堆结果
	public static void getMultiResultsByOneXpathPattern(byte[] usedata,
			String charset, String xpathpatter, Vector<String> results) {
		Object[] ns;
		try {
			String content = new String(usedata, charset);
			if (xpathpatter.startsWith("$")) {
				List<Object> cnt = JsonPath.read(
						content.substring(content.indexOf('{')), xpathpatter);
